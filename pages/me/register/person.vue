<template>
 ...
 -->
			...
			<view class="item">
				...
				<text class="cuIcon-phone" style="font-size: 23px;"></text>
			...
			<view class="item">
				...
				<text class="cuIcon-write" style="font-size: 23px;"></text>
				<view class="type">
					<input v-model="code" class="main-input-code" maxlength="6" type="number" placeholder="请输入验证码" />
					<span v-show="show" @click="handleClickCode">获取验证码</span>
			...
			<view class="item">
				...
				<text class="cuIcon-lock" style="font-size: 23px;"></text>
				<view class="type">
					<input v-model="passData" class="main-input-code" maxlength="20" type="password" placeholder="请设置六位密码" />
				</view>
			</view>
		</view>
		<view class="btn_login bg-red" @click="startRegister">注册</view>
		<!-- 底部信息 -->
		<view class="register_footer">
			<text @tap="isShowAgree" class="cuIcon" :class="showAgree?'cuIcon-radiobox':'cuIcon-round'">同意
			</text>
			<!-- 协议地址 -->
			<navigator url="/pages/me/register/xieyi/xieyi" open-type="navigate">《用户服务协议》</navigator>和
			<navigator url="/pages/me/register/yingsi/yingsi" open-type="navigate">《用户隐私协议》</navigator>
		</view>
	</view>
</template>

<script>
	import { GetMobileCode, UserRegister } from '@/api/comm.js'
	...
	export default {
		...
		methods: {
			...
			handleClickCode: function() {
				...
				// 获取验证码
				GetMobileCode(this.phoneData).then(res => {
					this.$queue.showToast(res.msg + `测试阶段--->${res.code}`);
				})
				// 倒计时
				const TIME_COUNT = 60;
				if (!this.timer) {
...
<style>
	@import url('../../../static/login/css/icon.css');
	@import url('../../../static/login/css/main.css');
